...
(** sum(latency) > 5 --> verNow = false (sum is ascending, > is ascending).
    Actually verify_now(Sum, Gt) = (ascending Sum = true) == (less Gt = false) =
    false. So the check is deferred to verify_policy.
...
let () =
  test "avg(cost)<10: empty history → ok (no division by zero)" ~expect_ok:1
    ~expect_err:0 test_avg_empty_history
...
let () =
  test "sorted(cost) asc: valid seq → verify_policy is noop (ok)" ~expect_ok:1
    ~expect_err:0 test_ascending_verify_noop

(* ---- check_each_group: grouped avg, two distinct keys ------------- *)

(** avg(cost) < 10 groupBy userId. Group userId=1: costs [4, 6] → avg=5 < 10 →
    ok. Group userId=2: costs [20] → avg=20 ≥ 10 → violation. Expected:
    verify_policy must check BOTH groups and report the error from group
    userId=2, even though group userId=1 is fine.
...
let () =
  test "grouped avg(cost)<10: group1 ok, group2 violates → error" ~expect_ok:0
    ~expect_err:1 test_grouped_avg_one_group_violates
...
let () =
  test "grouped: service without groupBy param is skipped → ok" ~expect_ok:1
    ~expect_err:0 test_grouped_skips_service_without_param
