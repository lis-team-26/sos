...
let make_service ?(params = []) name : Contract.AST.service =
  {
    name;
    params;
    returns = ("dummy", Utils.Types.TInt);
    precond = [];
    qos_postcond = ([], []);
    ok_postcond = ([], []);
    err_postcond = None;
...
let make_call svc ?(args = []) qos_pairs : invocation =
  let qos =
    List.fold_left
      (fun m (k, v) -> StringMap.add k (Typed.int v) m)
      StringMap.empty qos_pairs
  in
  { svc; actual_args = List.map Typed.int args; qos }

let service_map services =
  List.fold_left
    (fun m (s : Contract.AST.service) -> StringMap.add s.name s m)
...
let () =
  let policy =
    PC.init_policy
      ( Contract.AST.QosFieldOp (Contract.AST.Sum, "cost", Contract.AST.Lt, 100),
        None )
  in
  ...
  let calls =
    [
      make_call "S" [ ("cost", 60); ("latency", 0) ];
      make_call "S" [ ("cost", 50); ("latency", 0) ];
    ]
  in
...
let () =
  ...
  let calls =
    [
      make_call "S" [ ("cost", 30); ("latency", 0) ];
      make_call "S" [ ("cost", 40); ("latency", 0) ];
    ]
  in
...
let () =
  let policy =
    PC.init_policy
      ( Contract.AST.QosFieldOp
          (Contract.AST.Max, "latency", Contract.AST.Gt, 100),
        None )
  in
  let svc = make_service "S" in
  let smap = service_map [ svc ] in
...
let () =
  ...
  let calls =
    [
      make_call "S" [ ("latency", 80); ("cost", 0) ];
      make_call "S" [ ("latency", 150); ("cost", 0) ];
    ]
  in
...
let () =
  let policy =
    PC.init_policy
      ( Contract.AST.QosFieldOp (Contract.AST.Min, "cost", Contract.AST.Lt, 5),
        None )
  in
  let svc = make_service "S" in
  let smap = service_map [ svc ] in
  let calls =
    [
      make_call "S" [ ("cost", 10); ("latency", 0) ];
      make_call "S" [ ("cost", 8); ("latency", 0) ];
    ]
  in
...
let () =
  ...
  let calls =
    [
      make_call "S" [ ("latency", 10); ("cost", 0) ];
      make_call "S" [ ("latency", 20); ("cost", 0) ];
      make_call "S" [ ("latency", 30); ("cost", 0) ];
    ]
  in
...
let () =
  let policy =
    PC.init_policy
      ( Contract.AST.QosFieldOp (Contract.AST.Sum, "cost", Contract.AST.Lt, 50),
        Some "userId" )
  in
...
let () =
  ...
  let calls =
    [
      make_call "A" [ ("cost", 0); ("latency", 0) ];
      make_call "B" [ ("cost", 0); ("latency", 0) ];
    ]
  in
