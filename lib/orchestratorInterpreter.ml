...
let cast_to_int v =
  ...
  | None -> Symex.Result.error "Expected an integer"

let cast_to_bool v =
  match Typed.cast_checked v Typed.t_bool with
  | Some b -> Symex.Result.ok b
  | None -> Symex.Result.error "Expected a boolean"
...
let symb_eval_arithm_op v1 op v2 =
  ...
  | _ -> Symex.Result.error "Type error in arithmetic operation"
...
let symb_eval_bool_bin_op v1 op v2 =
  ...
  | _ -> Symex.Result.error "Type error in boolean operation"
...
let symb_eval_cmp_op v1 op v2 =
  ...
  | _ -> Symex.Result.error "Type error in comparison operation"
...
let rec symb_eval_expr env = function
  ...
  | EBinOp (e1, op, e2) ->
      ...
      let&++ result =
        ...
        | Lt | Le | Gt | Ge | Eq | Neq ->
            let++ cmp_result = symb_eval_cmp_op v1 op v2 in
            Typed.cast cmp_result
      in
      Typed.cast result
  ...
  | Assign (x, e) ->
      let&** t =
        (match StringMap.find_opt x state.private_env with
          ...
          | None -> Symex.Result.error (Fmt.str "Variable %s not declared" x))
        |> seal_error state
      in
      let& v =
        lift_fm
