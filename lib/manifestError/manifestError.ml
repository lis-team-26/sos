...
(* TODO: remove Z3 dependency and use Simple_smt library *)
open Z3
open Z3.Arithmetic

type error_result = { err_history : history; path_condition : symb_bool list }

(** Gets the id of a symbolic value generated by Soteria using [Symex.nondet].
*)
  ...
  let error_results =
    List.filter_map
      (fun (state, path_condition) ->
        ...
        | Some (Error (Err { err_history; cause })) ->
            Some (cause, { err_history; path_condition }))
      results
  in
  List.fold_left
...
let translate_bin_op ctx op e1 e2 =
  match op with
  | E.Eq -> Boolean.mk_eq ctx e1 e2
  | E.Le -> Arithmetic.mk_le ctx e1 e2
  | E.Lt -> Arithmetic.mk_lt ctx e1 e2
  | E.Neq -> Boolean.mk_not ctx (Boolean.mk_eq ctx e1 e2)
  | E.Ge -> Arithmetic.mk_ge ctx e1 e2
  | E.Gt -> Arithmetic.mk_gt ctx e1 e2
  | E.Add -> Arithmetic.mk_add ctx [ e1; e2 ]
  | E.Sub -> Arithmetic.mk_sub ctx [ e1; e2 ]
  | E.Mul -> Arithmetic.mk_mul ctx [ e1; e2 ]
  | E.Div -> Arithmetic.mk_div ctx e1 e2
  | E.And -> Boolean.mk_and ctx [ e1; e2 ]
  | E.Or -> Boolean.mk_or ctx [ e1; e2 ]

let translate_un_op = function E.Not -> Boolean.mk_not

let rec translate_assumption ctx name_map e =
  match e.it with
  | E.EInt i -> Integer.mk_numeral_i ctx i
  | E.EVar v -> StringMap.find v name_map
  | E.EBool true -> Boolean.mk_true ctx
  | E.EBool false -> Boolean.mk_false ctx
  | E.EUnOp (op, e) ->
      (translate_un_op op) ctx (translate_assumption ctx name_map e)
  | E.EBinOp (e1, op, e2) ->
      translate_bin_op ctx op
        (translate_assumption ctx name_map e1)
        (translate_assumption ctx name_map e2)
...
let rec translate_svalue ctx id_map exp =
  ...
  | Svalue.Unop (op, expr) -> (
      match op with
      | Not -> Boolean.mk_not ctx (translate_svalue ctx id_map expr))
...
let make_z3_constant ctx typ =
  ...
  | _ -> failwith "Receipts can't be a z3 constant")
    ctx symbol
...
let z3_find globals assumptions error_list =
  ...
  let translate_assumptions ctx vars expr_list =
    expr_list |> List.map (translate_assumption ctx vars) |> Boolean.mk_and ctx
  in
  let translate_conjunction ctx vars expr_list =
    ...
    |> Boolean.mk_and ctx
  in
  let assumptions = translate_assumptions ctx forall_vars_name assumptions in
  let forall_body =
    path_conditions
    |> List.map (fun pc ->
