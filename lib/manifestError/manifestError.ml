...
let rec translate_expr ctx id_map exp =
  ...
  | Svalue.Var v ->
     (if Svalue.is_bool_ty (exp |> Typed.type_ |> Typed.get_ty) then
       Boolean.mk_const
     else Integer.mk_const) ctx (IntMap.find (Soteria.Symex.Var.to_int v) id_map)
  ...
  | Svalue.Binop (op, expr1, expr2) ->
     let (e1, e2) = (translate_expr ctx id_map expr1, translate_expr ctx id_map expr2)
     in (match op with
  ...
  | Svalue.Nop (op, exprList) ->
           (match op with Distinct -> Boolean.mk_distinct ctx (List.map (translate_expr ctx id_map) exprList))
  | Svalue.Ite (expr1, expr2, expr3) ->
           Boolean.mk_ite ctx (translate_expr ctx id_map expr1) (translate_expr ctx id_map expr2) (translate_expr ctx id_map expr3)
let counter = ref 0

let make_z3_constant ctx typ =
  let symbol = (Symbol.mk_int ctx !counter)
  in
  let () = counter := !counter + 1
  in
  (symbol, (match typ with
...
let z3_find globals error_list =
  ...
  let forall_vars =
    ...
    |> List.map snd
    |> List.mapi
         (fun i t -> (i, make_z3_constant ctx t))
    |> IntMap.of_list
  in
  let forall_vars_count = IntMap.cardinal forall_vars
  in
  let path_conditions = List.map (fun x -> x.path_condition) error_list
  in
  let exists_vars path_condition =
    List.fold_left (fun set bexpr ->
        get_typed_vars bexpr
        |> IntMap.filter (fun id _ -> id >= forall_vars_count)
        |> IntMap.union (fun id t1 t2 -> if t1 == t2 then Some t1 else failwith "type mismatch in the same path condition") set
  ...
  in
  let union_vars ev =
    IntMap.union (fun _ _ _ -> failwith "initial and not initial symb values have the same id") ev forall_vars
    |> IntMap.map fst
  in
  let forall_body =
    path_conditions
    |> List.map (fun pc ->
           let ev = exists_vars pc
           in (ev,
               (pc
                |> List.map Typed.Expr.of_value
                |> List.map (translate_expr ctx (union_vars ev))
                |> Boolean.mk_and ctx)))
    |> List.map (fun (vars, conjunction) -> Quantifier.mk_exists_const ctx (vars |> IntMap.to_list |> List.map snd |> List.map snd) conjunction (Some 1) [] [] None None)
    |> List.map Quantifier.expr_of_quantifier
    |> Boolean.mk_or ctx
  in
  let final_formula = Quantifier.mk_forall_const ctx (forall_vars |> IntMap.to_list |> List.map snd |> List.map snd) forall_body (Some 1) [] [] None None
  (*in
