...
type error_result = {err_stack : stack; rev_function_map : IntSet.t IntMap.t; path_condition : symb_bool list }
...
let get_toplevel_vars_symbolic_value =
  function 
  ...
  StringMap.fold (
      ...
      |>
        Seq.fold_left (
            ...
            let value = get_toplevel_vars_symbolic_value return
            in
            let argsSet =
              args
              |> List.map get_toplevel_vars
              |> List.fold_left IntSet.union IntSet.empty
            in
            if IntSet.cardinal value != 1 then
              failwith "There should be a single nondet as the domain of the function map"
            else
              IntMap.update (IntSet.choose value) (function
                  | None -> Some argsSet
                  | Some args -> Some (IntSet.union args argsSet)) rev_map
          ) rev_map
  ...
  let error_results =
    List.filter_map
      (fun (state, path_condition) ->
        ...
        | Some (Error (Err {err_stack; function_envs; cause }))-> Some (cause, {err_stack; rev_function_map = (reverse_function_map function_envs); path_condition}))
      results
  in
  List.fold_left
    (fun error_cause_map (cause, error) ->
      ErrorCauseMap.update cause
        (function
          | None -> Some [ error ] | Some l -> Some (error :: l))
        error_cause_map)
...
let expand_vars variables reverse_function_map =
  let rec ev_rec vars =
    IntSet.fold (fun variable collected ->
...
let get_all_vars expr reverse_function_map =
  let vars = get_toplevel_vars expr
  in expand_vars vars reverse_function_map

let build_symex_assertion formula =
  let var_types = get_toplevel_typed_vars formula
  in
  let var_idx =
    Seq.ints 0
  |> Seq.take (match IntMap.max_binding_opt var_types with
...
let split_heuristic global_vars error_list =
  ...
  let error_list = List.filter splittable error_list
  in
  let formula =
    error_list
    |> List.map (fun error ->
           let depends_only_on_globals = (*set of symb values that depend only on globals*)
             error.rev_function_map
             |> IntMap.bindings
             |> List.map fst
             |> IntSet.of_list
             (*if all the leaves are globals, then the root depends only on globals*)
             |> IntSet.filter (fun var -> IntSet.subset (expand_vars (IntSet.singleton var) error.rev_function_map) global_vals_set)
             |> IntSet.union global_vals_set (*globals values also depend on globals, of course*)
           in
           List.filter (fun expr -> IntSet.subset (get_toplevel_vars expr) depends_only_on_globals) error.path_condition)
    |> List.map (List.fold_left Typed.and_ Typed.v_true)
    |> List.fold_left Typed.or_ Typed.v_false
  in
  let results =
    Symex.run
      (build_symex_assertion formula)
