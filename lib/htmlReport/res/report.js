(function () {
  var fallbackData = {
    counts: { total: 0, success: 0, error: 0, incomplete: 0 },
    results: [],
    errorGroups: [],
    stats: { metrics: [] },
    manifestErrors: []
  ...
  function renderInvocations(invocations) {
    ...
    invocations.forEach(function (invocation) {
      ...
      append(tr, append(element("td"), valueBox(invocation.returned)));
      append(tr, append(element("td"), valueBox(invocation.successful)));
  ...
  function renderErrorIndex() {
    ...
    var groups = data.errorGroups && data.errorGroups.length ? data.errorGroups : groupErrorsFromResults();
  ...
  function groupErrorsFromResults() {
    ...
    data.results.forEach(function (result) {
      ...
      if (!byKey[key]) {
        byKey[key] = { error: result.error, count: 0, results: [] };
  ...
  function renderErrorGroupBody(group) {
    ...
    var wrap = element("div", "table-responsive");
    var table = element("table", "table table-sm align-middle mb-0");
    table.innerHTML = "<thead><tr><th>Result</th><th>Path conditions</th><th>Invocations</th></tr></thead>";
    var tbody = element("tbody");
    append(table, tbody);
    append(wrap, table);
