...
let range_ maybe_domain l h =
  ...
  match maybe_domain with None -> in_range | Some d -> C.inter d in_range
...
module Parse = struct
  ...
  module Bracket = struct
    ...
    let interpret maybe_domain { negated; elements } =
      ...
      in
      let s = List.fold_left fold_fun C.empty elements in
      if negated then C.diff (any_ maybe_domain) s else s
...
let parse ?domain s =
  ...
  with Parse.Error e -> raise (Parse_error (s, e))
